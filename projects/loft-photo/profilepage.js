...
export default {
  async setUser(user) {
    ...
    const photosTab = document.querySelector('.component-profile-node-photos');
    const friendsTab = document.querySelector('.component-profile-node-friends');

    this.user = user;
    photoComp.style.backgroundImage = `url('${user.photo_100}')`;
    nameComp.innerText = `${user.first_name ?? ''} ${user.last_name ?? ''}`;
    const node = localStorage.getItem('loft-photo-profile-node') ?? '1';
  ...
  handleEvents() {
    document
      ...
      .addEventListener('click', async (e) => {
        if (e.target.classList.contains('component-user-photo')) {
          ...
          const size = model.findSize(photo);
          const stats = await model.photoStats(photoId);
    ...
    document
      .querySelector('.component-profile-node-photos')
